...
from AutoPPT.slide_generator import HTMLToPPTXParser, PPTXGenerator
...
def convert_from_json(json_file: str, output_file: str = None):
    ...
    logger.info(f"   📂 讀取 JSON 數據：{json_file}")
...
def convert_from_html(html_file: str, output_file: str = None):
    ...
    logger.info(f"   ⚠️ 使用舊版 HTML → PPTX 轉換")
...
def main():
    ...
    # 優先使用 JSON
    if json_files:
        logger.info(f"   ✅ 找到 {len(json_files)} 個 JSON 數據文件")

        if len(json_files) > 1:
            logger.info("   📋 可用的 JSON 文件：")
            for i, f in enumerate(json_files, 1):
                logger.info(f"   {i}. {f}")

            choice = input("   請選擇要轉換的文件編號（直接按 Enter 選擇第一個）: ").strip()
        ...
        logger.info(f"   📂 選擇的文件：{json_file}")
        convert_from_json(json_file)

    elif html_files:
        logger.info(f"   ⚠️ 只找到 HTML 文件（建議使用 JSON 方式）")

        if len(html_files) > 1:
            logger.info("   📋 可用的 HTML 文件：")
