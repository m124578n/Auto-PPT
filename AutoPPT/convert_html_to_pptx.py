...
def convert_from_html(html_file: str, output_file: str = None):
    ...
    print(f"\n⚠️ 使用舊版 HTML → PPTX 轉換")
    print(f"   建議使用 JSON → PPTX 轉換以獲得更好的效果")
...
def verify_pptx(output_file: str):
    ...
    if os.path.exists(output_file):
        ...
        print(f"\n📦 檔案資訊：")
        print(f"   路徑：{os.path.abspath(output_file)}")
        print(f"   大小：{file_size:,} bytes ({file_size/1024/1024:.2f} MB)")
        
        try:
            verify_prs = Presentation(output_file)
            print(f"   幻燈片數：{len(verify_prs.slides)}")
        except Exception as e:
            print(f"   ⚠️ 驗證失敗：{e}")


def main():
    print("🔄 HTML → PPTX 轉換器（重構版）")
    print("=" * 60)
    print(f"📊 支援的 Slide 類型：{', '.join(SlideTypeRegistry.all_types())}")
    print("=" * 60)
